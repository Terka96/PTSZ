...
# Mutation parameters
SWAP_ELEMENT_CHANCE = numpy.arange(0.1, 0.9, 0.2)  # Chance to swap the job
CHANGE_R_CHANCE = numpy.arange(0.1, 0.9, 0.2)  # Chance to change R position
...
inst_col = [10, 20, 50] if sys.argv[1] is None else [int(sys.argv[1])]

#todo: wyniki muszą pochodzić z kilku pomiarów i być uśredniane
for s in inst_col: #, 20, 50, 100, 200, 500, 1000]:
    iter = 0
    speed = 0
                    ...
                    for mut_cha in MUTATION_CHANCE:
                        for swap_cha in SWAP_ELEMENT_CHANCE:
                            for change_cha in CHANGE_R_CHANCE:
                                ...
                                speed = (speed * (iter-1) + time.time() - start_time)/iter
                                print("\n:: ~{:.2f} min left ::".format(speed*(iterations-iter)/60))
